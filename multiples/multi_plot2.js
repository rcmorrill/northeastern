...
var multi_m = {t:20,r:30,b:40,l:30},
    multi_w = $('#attribute_plot').width()/4 - multi_m.l - multi_m.r
    multi_h = 200;
...
var NullGen = d3.svg.area()
	...
	.y1(multi_h);
		...
		function drawattribute(attribute_data){

		var draw = d3.select('#attribute_plot').selectAll('svg')
			.data(attributetypes)
			.enter()
...
var plot2 = d3.select('.custom-tooltip')
				.append('svg')
				.attr('width',tipW)
				.attr('height',tipH)
...
d3.selectAll('.anchor').attr('opacity',0)
	...
	function tooltip(tip){
	    ...
	    tip.on('mouseenter',function(d){
            ...
            tooltip.select('p').html("Percent of employers who feel"+"<br>"+" graduates do well (5 or 6)"+ "<br>" + "with respect to:" +"<br><br>" + "<strong>" + d.key + "</strong>");
	    ...
	    .on('mousemove', function(d){
	            var xy = d3.mouse(document.getElementById('attribute_plot'));
		...
		d3.csv ('multiples/data/attribute_plot.csv',parse,dataLoaded);
		...
		function dataLoaded(err, attribute_data){

		attributetypes = d3.nest()
			.key(function(d) {return d.question})
			.entries(attribute_data);

		drawattribute(attribute_data);
